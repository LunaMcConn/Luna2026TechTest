...
| **TC-18** | Double decimal number | `12.34.56` | Out-of-range message |
| **TC-19** | Empty value | `?number=` | No converted-number element rendered |
| **TC-20** | Parameter omitted entirely | `/` | No converted-number element rendered |
...
## How to run
To run the test navigate to the test directory and run the test command, like:
```bash
cd HelloWorldWeb.Tests
dotnet test
```